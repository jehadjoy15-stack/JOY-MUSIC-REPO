    ...
    private suspend fun handleFrame(text: String) {
        ...
        when (op) {
            HELLO -> {
                ...
                if (token.isNotBlank()) {
                    Timber.tag(TAG).i("handleFrame: HELLO received, auto-sending IDENTIFY/RESUME")
                    if (_sessionId != null && _currentSeq > 0) {
                        try {
                            resume(_sessionId!!, _currentSeq, token)
                        } catch (e: Throwable) {
                            identify(token)
                        }
                    } else {
