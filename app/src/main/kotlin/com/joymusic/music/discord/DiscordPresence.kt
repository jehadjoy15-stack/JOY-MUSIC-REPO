...
object DiscordPresence {
    ...
    private fun activityToJson(activity: ActivityPayload): JSONObject {
        val obj = JSONObject()
        obj.put("name", activity.name.ifEmpty { "JOY MUSIC" })
        obj.put("type", activity.type)
        obj.put("application_id", com.joymusic.music.BuildConfig.DISCORD_APP_ID.toString())
        ...
        if (validLarge != null || validSmall != null || !activity.largeText.isNullOrEmpty() || !activity.smallText.isNullOrEmpty()) {
            ...
            validLarge?.let { assets.put("large_image", it) }
            activity.largeText?.let { assets.put("large_text", it) }
            validSmall?.let { assets.put("small_image", it) }
            activity.smallText?.let { assets.put("small_text", it) }
            obj.put("assets", assets)
        }

        // In Gateway op 3 PRESENCE_UPDATE, buttons is string[] (labels only) and metadata.button_urls contains the URLs.
